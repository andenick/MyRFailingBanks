...
p <- ggplot(panel_data %>% filter(!is.na(leverage))) +
  ...
  labs(title = "Regressor Stability") +
  theme_failing_banks()
ggsave(file.path(output_dir, "51_regressor_stability.png"), p, width = 12, height = 8, dpi = 300, bg = "white")
cat("Saved: 51_regressor_stability.png
")
