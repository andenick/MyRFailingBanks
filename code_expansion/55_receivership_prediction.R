# Script 55: Receivership Prediction
library(tidyverse)
library(here)
library(scales)
...
p <- ggplot(panel_data %>% filter(!is.na(leverage))) +
  ...
  labs(title = "Receivership Prediction") +
  theme_failing_banks()
ggsave(file.path(output_dir, "55_receivership_prediction.png"), p, width = 12, height = 8, dpi = 300, bg = "white")
cat("Saved: 55_receivership_prediction.png
")
