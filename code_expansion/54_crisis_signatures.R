...
p <- ggplot(panel_data %>% filter(!is.na(leverage))) +
  ...
  labs(title = "Crisis Signatures") +
  theme_failing_banks()
ggsave(file.path(output_dir, "54_crisis_signatures.png"), p, width = 12, height = 8, dpi = 300, bg = "white")
cat("Saved: 54_crisis_signatures.png
")
